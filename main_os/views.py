...
def report(request):
    if request.user.is_authenticated:
        ...
        worksheet.merge_range(rows+3,col+6,rows+3,col+7,'Жунушалиев Д.Т.', format5)
...
def cards_pdf(request):
    if request.user.is_authenticated:
        ...
        #response = cards_table_create(request)
        #return  render(request, 'main_os/cards_pdf_template.html', response, content_type='application/pdf')
    else:
        return redirect('user_login')
